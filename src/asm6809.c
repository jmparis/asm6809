...
int main(int argc, char **argv) {
		...
		case 'P':
			{
				...
				if (errno != 0 || v < 2 || v > 255) {
					error_report(error_type_fatal, "invalid value for max-passes");
					error_print_list();
	...
	if (optind >= argc) {
		error_report(error_type_fatal, "no input files");
		error_print_list();
		tidy_up_and_exit(EXIT_FAILURE);
	}
	...
	if (listing_filename) {
		...
		if (listf) {
			...
			error_report(error_type_fatal, "%s: %s", listing_filename, strerror(errno));
		}
	}
	...
	if (exec_option) {
		...
		if (!n) {
			...
			if (tmp) {
				...
				error_report(error_type_fatal, "exec symbol '%s' not defined", exec_option);
			}
			n = node_new_int(v);
		}
	...
	if (output_filename) {
		...
		case OUTPUT_INTEL_HEX:
			...
			break;
		default:
			error_report(error_type_fatal, "internal: unexpected output format");
			break;
		}
	}
	...
	if (exports_filename) {
		...
		if (expf) {
			...
			error_report(error_type_fatal, "%s: %s", exports_filename, strerror(errno));
		}
	}
	...
	if (symbol_filename) {
		...
		if (symf) {
			...
			error_report(error_type_fatal, "%s: %s", symbol_filename, strerror(errno));
		}
	}
...
"Copyright (C) 2025 Ciaran Anscomb\n"
