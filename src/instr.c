...
void instr_inherent(struct opcode const *op, struct node const *args) {
	...
	if (nargs != 0) {
		error_report(error_type_syntax, "unexpected argument");
	}
...
void instr_immediate(struct opcode const *op, struct node const *args) {
	...
	if (nargs != 1) {
		error_report(error_type_syntax, "invalid number of arguments");
		return;
	}
	section_emit_op(op->immediate);
...
void instr_rel(struct opcode const *op, struct node const *args) {
	...
	if ((op->type & OPCODE_EXT_TYPE) == OPCODE_REL8) {
		if (rel8v)
			error_report(error_type_out_of_range, "8-bit relative value out of range");
		section_emit_uint8(rel8);
	} else {
		if (!rel8v && node_attr_of(arga[0]) != node_attr_16bit)
			error_report(error_type_inefficient, "16-bit relative value could be represented in 8 bits");
...
static _Bool off_type_compatible(enum off_type off_type, _Bool pcr, struct node const *n) {
	...
	case off_type_5bit:
		...
		return ntype == node_type_empty;
	case off_type_8bit:
		if (pcr && ntype == node_type_empty) {
			int64_t val_int = to_rel16(0 - (cur_section->pc + 2));
...
static void instr_indexed2(_Bool indirect, struct node const *arg0, struct node const *arg1) {
	...
	if (arg0_type == node_type_int || (pcr && arg0_type == node_type_empty)) {
		if (pcr) {
	...
	if (indirect) {
		...
		if (idx_indirect == idx_indirect_illegal)
			error_report(error_type_illegal, "illegal indirect indexed mode");
...
void instr_indexed(struct opcode const *op, struct node const *args, int imm8_val) {
	...
	if (nargs == 1) {
		...
		if (!indirect)
			error_report(error_type_illegal, "illegal indexed addressing form");
...
void instr_stack(struct opcode const *op, struct node const *args, enum reg_id stack) {
	...
	for (int i = 0; i < nargs; i++) {
		...
		if (node_type_of(arga[i]) != node_type_reg) {
			error_report(error_type_syntax, "invalid argument");
			return;
		}
...
invalid_register:
	error_report(error_type_syntax, "invalid register in stack operation");
	return;
}
	...
	for (int i = 0; i < 2; i++) {
		...
		if (type[i] == node_type_reg) {
			...
			if (v == 0 && asm6809_options.isa == asm6809_isa_6309) {
				...
				error_report(error_type_illegal, "numerical values used in inter-register op");
				if (v < 0 || v > 15) {
					error_report(error_type_out_of_range, "numeric value for register out of range");
				}
				nibble[i] = v & 15;
			...
			error_report(error_type_syntax, "invalid register in inter-register op");
			return;
		}
	}
	if ((size[0] & size[1]) == 0) {
		error_report(error_type_illegal, "register size mismatch in inter-register op");
	}
...
void instr_tfm(struct opcode const *op, struct node const *args) {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (type[i] == node_type_reg) {
			...
			if (v < 0 || v > 4) {
				error_report(error_type_syntax, "invalid register in TFM op");
				return;
			}
			nibble[i] = v;
		} else if (type[i] == node_type_int) {
			int v = arga[i]->data.as_int;
			error_report(error_type_illegal, "numerical values used in TFM op");
			if (v < 0 || v > 4) {
				error_report(error_type_out_of_range, "numeric value for register out of range");
			}
			nibble[i] = v & 15;
	...
	if (attr[0] == node_attr_postinc && attr[1] == node_attr_postinc) {
		...
		error_report(error_type_syntax, "invalid TFM mode");
		return;
	}
...
void instr_reg_mem(struct opcode const *op, struct node const *args) {
	...
	case node_type_int:
		...
		error_report(error_type_illegal, "numerical value used in place of register");
		break;
	case node_type_reg:
	...
	if (node_type_of(arga[1]) == node_type_int) {
		if (arga[1]->data.as_int < 0 || arga[1]->data.as_int > 7) {
			error_report(error_type_out_of_range, "bad source bit");
		} else {
	...
	if (node_type_of(arga[2]) == node_type_int) {
		if (arga[2]->data.as_int < 0 || arga[2]->data.as_int > 7) {
			error_report(error_type_out_of_range, "bad destination bit");
		} else {
...
invalid_register:
	error_report(error_type_syntax, "invalid register in register-memory operation");
	return;
}
