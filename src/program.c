...
struct prog *prog_new_macro(const char *name) {
	if (prog_macro_by_name(name)) {
		error_report(error_type_syntax, "attempt to redefined macro '%s'", name);
		return NULL;
	}
...
void prog_ctx_free(struct prog_ctx *ctx) {
	...
	if (prog_ctx_stack->data != ctx) {
		...
		if (last_ctx->prog->type == prog_type_macro) {
			error_report(error_type_syntax, "unfinished macro");
...
static void print_symbol(const char *key, FILE *f) {
	...
	if (sym) {
		...
		default:
			error_report(error_type_syntax, "can't export symbol type");
			node_free(n);
			return;
		case node_type_int:
