...
Copyright 2013-2024 Ciaran Anscomb
 ...
 *
 * - phase: current pc/dp combination, stacked by PHASE/DEPHASE pseudo-ops
 *
 * - phase.pc: Current Program Counter.  May be modified without emitting data
 *   (e.g., by ORG or RMB), so if found not to match the current span's org +
 *   size, a new span needs to be created.
 *
 * - phase.dp: Direct Page.  Each section gets its own idea of what the direct
 *   page is, as specified by SETDP.
 *
...
/* Push a new phase to a section */

void section_push_phase(struct section *, unsigned pc);

/* Pop phase off a section's phase stack */

void section_pop_phase(struct section *);
