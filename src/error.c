...
static void verror_report(enum error_type type, const char *fmt, va_list ap) {
...
void error_report(enum error_type type, const char *fmt, ...) {
	va_list ap;
	va_start(ap, fmt);
	verror_report(type, fmt, ap);
	va_end(ap);
}
...
void error_abort(const char *fmt, ...) {
	va_list ap;
	va_start(ap, fmt);
	verror_report(error_type_fatal, fmt, ap);
	va_end(ap);
	error_print_list();
	exit(EXIT_FAILURE);
}
