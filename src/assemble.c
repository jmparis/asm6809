...
static void pseudo_phase(struct prog_line *);
static void pseudo_dephase(struct prog_line *);
...
static void pseudo_assert(struct prog_line *);
...
static struct pseudo_op pseudo_label_ops[] = {
	...
	{ .name = "phase", .handler = &pseudo_phase },
	{ .name = "dephase", .handler = &pseudo_dephase },
...
static struct pseudo_op pseudo_ops[] = {
	{ .name = "assert", .handler = &pseudo_assert },
...
static int verify_num_args(struct node *args, int min, int max, const char *op) {
	...
	if (max < 0) {
		if (nargs >= min)
			return nargs;
		error_report(error_type_syntax, "'%s' requires at least %d argument%s",
		      op, min, (min == 1) ? "" : "s");
		return -1;
	}
	if (nargs >= min && nargs <= max)
		return nargs;
	if (min == max) {
		error_report(error_type_syntax, "'%s' requires exactly %d argument%s",
		      op, min, (min == 1) ? "" : "s");
	} else {
		error_report(error_type_syntax, "'%s' requires between %d and %d arguments",
		      op, min, max);
	}
	return -1;
}
...
static int64_t have_int_optional(struct node *args, int aindex, const char *op, int64_t in) {
	...
	default:
		error_report(error_type_syntax,
...
void assemble_prog(struct prog *prog, unsigned pass) {
	if (prog_depth >= asm6809_options.max_program_depth) {
		error_report(error_type_fatal, "maximum program depth exceeded");
		return;
	}
	asm_pass = pass;
	prog_depth++;
	...
	while (!prog_ctx_end(ctx)) {
		...
		if (!cond_excluded && n_line.opcode && 0 == c_strcasecmp("endm", n_line.opcode->data.as_string)) {
			if (defining_macro_level == 0) {
				error_report(error_type_syntax, "ENDM without beginning MACRO");
				goto next_line;
			}
			defining_macro_level--;
			if (defining_macro_level == 0) {
		...
		if (n_line.opcode && 0 == c_strcasecmp("elsif", n_line.opcode->data.as_string)) {
			listing_add_line(-1, 0, NULL, l->text);
			if (!cond_list) {
				error_report(error_type_syntax, "ELSIF without IF");
			} else if ((intptr_t)cond_list->data == cond_state_else) {
				error_report(error_type_syntax, "repeated ELSE");
			} else {
				if (!cond_excluded) {
					cond_excluded = cond_list;
		...
		if (n_line.opcode && 0 == c_strcasecmp("else", n_line.opcode->data.as_string)) {
			listing_add_line(-1, 0, NULL, l->text);
			if (!cond_list) {
				error_report(error_type_syntax, "ELSE without IF");
			} else if ((intptr_t)cond_list->data == cond_state_else) {
				error_report(error_type_syntax, "repeated ELSE");
			} else if (cond_excluded == cond_list) {
		...
		if (n_line.opcode && 0 == c_strcasecmp("endif", n_line.opcode->data.as_string)) {
			listing_add_line(-1, 0, NULL, l->text);
			if (!cond_list) {
				error_report(error_type_syntax, "ENDIF without IF");
				goto next_line;
			} else if (cond_excluded == cond_list) {
				cond_excluded = NULL;
			}
		...
		if (op) {
			...
			if (op->type == OPCODE_INHERENT) {
				...
				error_report(error_type_syntax, "invalid addressing mode");
			}
			int nbytes = cur_section->pc - old_pc;
			listing_add_line(old_pc & 0xffff, nbytes, cur_section->span, l->text);
			goto next_line;
		...
		error_report(error_type_syntax, "unknown instruction '%s'", n_line.opcode->data.as_string);
...
next_line:
	...
	if (cond_list) {
		...
		error_report(error_type_syntax, "IF not matched with ENDIF");
	}

	// XXX should we warn here?  Implies use of PHASE without bracketing
	// DEPHASE.
	while (cur_section->phase_stack) {
		section_pop_phase(cur_section);
	}
...
static void set_label(struct node *label, struct node *value, _Bool changeable) {
	...
	default:
		error_report(error_type_syntax, "invalid label type");
		break;
	case node_type_undef:
		break;
	case node_type_int:
...
static void pseudo_org(struct prog_line *line) {
	...
	if (new_pc >= 0) {
		...
		for (struct slist *iter = cur_section->phase_stack; iter; iter = iter->next) {
			struct section_phase *phase = iter->data;
			cur_section->put -= phase->pcoff;
		}
	}
...
/* PHASE.  Offset PC without affecting PUT address.  Nestable. */

static void pseudo_phase(struct prog_line *line) {
	if (verify_num_args(line->args, 1, 1, "PHASE") < 0)
		return;
	// Any label on this line should get the address using the previous offset.
	unsigned label_addr = cur_section->put;
	if (cur_section->phase_stack) {
		struct section_phase *phase = cur_section->phase_stack->data;
		label_addr += phase->pcoff;
	}
	long new_pc = have_int_required(line->args, 0, "PHASE", cur_section->pc);
	section_push_phase(cur_section, new_pc);
	set_label(line->label, node_new_int(label_addr), 0);
	listing_add_line(new_pc & 0xffff, 0, NULL, line->text);
}

/* DEPHASE.  Return to previous PC without affecting PUT address. */

static void pseudo_dephase(struct prog_line *line) {
	if (!cur_section->phase_stack) {
		error_report(error_type_syntax, "DEPHASE without PHASE");
		return;
	}
	section_pop_phase(cur_section);
	// Any label on this line should get the address using the previous offset.
...
static void pseudo_section(struct prog_line *line) {
	...
	if (!n) {
		error_report(error_type_syntax, "invalid argument to SECTION");
		return;
	}
...
/* ASSERT.  Evaluates the first argument as an int, and flags an error if it is
 * zero.  An optional second string argument is appended to the error text. */

static void pseudo_assert(struct prog_line *line) {
	int nargs = verify_num_args(line->args, 1, 2, "ASSERT");
	if (nargs < 0) {
		return;
	}
	int cond = have_int_required(line->args, 0, "ASSERT", 0);
	if (!cond) {
		const char *msg = NULL;
		struct node **arga = node_array_of(line->args);
		if (nargs > 1 && node_type_of(arga[1]) == node_type_string) {
			msg = arga[1]->data.as_string;
		}
		if (msg) {
			error_report(error_type_inconsistent, "ASSERT failed: %s", msg);
		} else {
			error_report(error_type_inconsistent, "ASSERT failed");
		}
	}
}
...
static void pseudo_put(struct prog_line *line) {
	...
	if (new_put < 0) {
		error_report(error_type_out_of_range, "invalid negative address for PUT");
		return;
	}
...
static void pseudo_rzb(struct prog_line *line) {
	...
	if (count < 0) {
		error_report(error_type_out_of_range, "negative count for RZB");
	}
...
static void pseudo_fill(struct prog_line *line) {
	...
	if (count < 0) {
		error_report(error_type_out_of_range, "negative count for FILL");
	}
	for (long i = 0; i < count; i++)
		section_emit_uint8(fill);
}
...
static void pseudo_rmb(struct prog_line *line) {
	...
	if (count < 0) {
		error_report(error_type_out_of_range, "negative argument to RMB");
		return;
	}
	section_skip(count);
}
...
static void pseudo_align(struct prog_line *line) {
	...
	if (align < 1) {
		error_report(error_type_out_of_range, "invalid argument to ALIGN");
		return;
	}
...
static void pseudo_include(struct prog_line *line) {
	...
	if (node_type_of(arga[0]) != node_type_string) {
		error_report(error_type_syntax, "invalid argument to INCLUDE");
		return;
	}
...
static void pseudo_includebin(struct prog_line *line) {
	...
	struct node **arga = node_array_of(line->args);
	if (node_type_of(arga[0]) != node_type_string) {
		error_report(error_type_syntax, "invalid argument to INCLUDEBIN");
		return;
	}
	FILE *f = fopen(arga[0]->data.as_string, "rb");
	if (!f) {
		error_report(error_type_fatal, "file not found: %s", arga[0]->data.as_string);
		return;
	}
	int c;
	while ((c = fgetc(f)) != EOF) {
...
static void pseudo_macro(struct prog_line *line) {
	...
	case node_type_string:
		...
		break;
	default:
		error_report(error_type_syntax, "missing or invalid macro name");
		return;
	}
	struct prog *macro = prog_macro_by_name(name);
	if (macro) {
		if (macro->pass == asm_pass)
			error_report(error_type_syntax, "macro '%s' redefined", name);
		return;
	}
...
static void pseudo_endm(struct prog_line *line) {
	...
	struct node **arga = node_array_of(line->args);
	if (nargs > 1) {
		error_report(error_type_syntax, "invalid number of arguments to ENDM");
		return;
	}
	if (nargs > 0 && node_type_of(arga[0]) != node_type_string) {
		error_report(error_type_syntax, "invalid argument to ENDM");
		return;
	}
	if (!defining_macro_ctx)
		return;
