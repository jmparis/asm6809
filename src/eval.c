...
struct node *eval_node(struct node *n) {
	...
	case node_type_oper:
		...
		error_report(error_type_fatal, "internal: bad number of args (%d) for operator", n->data.as_oper.nargs);
		return NULL;
	...
	error_report(error_type_fatal, "internal: eval of unhandled type %d", n->type);
	return NULL;
}
...
static struct node *eval_node_oper_1(struct node *n) {
	...
	default:
		error_report(error_type_fatal, "internal: unknown 1-arg operator %d", n->data.as_oper.oper);
		break;
	}
...
static struct node *eval_node_oper_2(struct node *n) {
	...
	default:
		error_report(error_type_fatal, "internal: unknown 2-arg operator %d", n->data.as_oper.oper);
		break;
	}
