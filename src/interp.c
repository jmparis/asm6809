...
void interp_push(struct node *n) {
	...
	case node_type_array:
		break;
	default:
		error_report(error_type_fatal, "internal: pushing non-array onto interp stack");
		return;
	}
...
void interp_pop(void) {
	if (!interp_stack) {
		error_report(error_type_fatal, "internal: popping off empty interp stack");
		return;
	}
...
struct node *interp_get(int index) {
	if (!interp_stack) {
		error_report(error_type_syntax, "no positional variables on stack");
		return NULL;
	}
	struct node *args = interp_stack->data;
	int nargs = args ? args->data.as_array.nargs : 0;
	if (index < 1 || index > nargs) {
		error_report(error_type_syntax, "invalid positional variable: %d", index);
		return NULL;
	}
