...
void section_push_phase(struct section *sect, unsigned pc) {
	assert(sect != NULL);
	struct section_phase *phase = xmalloc(sizeof(*phase));
	*phase = (struct section_phase){0};
	phase->pcoff = pc - sect->pc;
	phase->dp = sect->dp;
	sect->phase_stack = slist_prepend(sect->phase_stack, phase);
	sect->pc = pc;
}

void section_pop_phase(struct section *sect) {
	assert(sect != NULL);
	assert(sect->phase_stack != NULL);
	struct section_phase *phase = sect->phase_stack->data;
	sect->phase_stack = slist_remove(sect->phase_stack, phase);
	sect->pc -= phase->pcoff;
	sect->dp = phase->dp;
	free(phase);
}
...
static void verify_section(void *key, void *value, void *data) {
	...
	if (sect->last_pc != sect->pc) {
		...
		error_report(error_type_inconsistent, NULL);
	}
}
	...
	for (struct slist *l = spans; l && l->next; l = l->next) {
		do {
			...
			if (span_end > nspan->put) {
				error_report(error_type_data, "data at $%04X overlaps data at $%04X", span->put, nspan->put);
...
static void section_emit(uint8_t const *buf, int nbytes) {
	...
	if (cur_section->pc < 0) {
		error_report(error_type_out_of_range, "assembling to negative address");
	}
	...
	if (cur_section->pc > 0x10000) {
		error_report(error_type_out_of_range, "assembling beyond addressable memory");
	}
}
